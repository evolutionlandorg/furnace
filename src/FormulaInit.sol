...
contract FormulaInit is FurnaceSettingIds {
	...
	function initFormula0(address token) private {
		bytes32 name = "合金镐";	
		bytes memory meta = abi.encodePacked(uint16(1), uint16(1), true, uint128(4 * RATE_DECIMALS), uint128(4 * RATE_DECIMALS));
		bytes32[] memory majors = new bytes32[](1);
		address[] memory minors = new address[](1);
		uint256[] memory limits = new uint256[](1);
		bytes memory majorData = abi.encodePacked(registry.addressOf(CONTRACT_ERC721_GEGO), uint16(0), uint16(1), bytes8(0)); 
	...
	function initFormula5(address token) private {
		...
		address[] memory minors = new address[](1);
		uint256[] memory limits = new uint256[](1);
		bytes memory majorData = abi.encodePacked(registry.addressOf(CONTRACT_OBJECT_OWNERSHIP), uint16(1), uint16(1), bytes8(0)); 
		majors[0] = abi.decode(majorData, (bytes32));
		minors[0] = token;
		limits[0] = ((200 * UINT) << 128) | (350 * UINT);
		address formula = registry.addressOf(CONTRACT_FORMULA);
	...
	function initFormula10(address token) private {
		bytes32 name = "人力钨钢钻机";	
		bytes memory meta = abi.encodePacked(uint16(1), uint16(2), true, uint128(7 * RATE_DECIMALS), uint128(8 * RATE_DECIMALS));
		bytes32[] memory majors = new bytes32[](1);
		address[] memory minors = new address[](1);
		uint256[] memory limits = new uint256[](1);
		bytes memory majorData = abi.encodePacked(registry.addressOf(CONTRACT_OBJECT_OWNERSHIP), uint16(1), uint16(2), bytes8(0)); 
		majors[0] = abi.decode(majorData, (bytes32));
		minors[0] = token;
	...
	function initFormula15(address token) private {
		bytes32 name = "人力金刚钻机";	
		bytes memory meta = abi.encodePacked(uint16(1), uint16(3), true, uint128(30 * RATE_DECIMALS), uint128(70 * RATE_DECIMALS));
		bytes32[] memory majors = new bytes32[](1);
		address[] memory minors = new address[](1);
		uint256[] memory limits = new uint256[](1);
		bytes memory majorData = abi.encodePacked(registry.addressOf(CONTRACT_OBJECT_OWNERSHIP), uint16(1), uint16(3), bytes8(0)); 
		majors[0] = abi.decode(majorData, (bytes32));
		minors[0] = token;
