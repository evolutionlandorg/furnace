...
contract Furnace is DSStop, FurnaceSettingIds {
	...
	event Disenchant(address indexed owner, uint256 indexed disenchantTokenId, uint256 tokenId, address[] tokens, uint256[] amounts);
	...
	function _smelt(uint256 _index, uint256 _major, address[] memory _tokens, uint256[] memory _amounts) internal {
		require(_index > 0  && _index < formula.length(), "Invalid index.");
		require(_major > 0, "Invalid major token id.");
		require(msg.sender == ownership.ownerOf(_major), "Only owner can smelt.");
        (, , , , uint16 majorIndex, bytes32[] memory tokens, uint256[] memory mins, uint256[] memory maxs) = formula.at(_index);
	...
	function _smeltItem(uint256 _index, uint256 _prefer, uint256 _rate, uint256 _major, address[] memory _tokens, uint256[] memory _amounts, address _owner) internal {
        (, uint16 class, uint16 grade, bool canDisenchant, , , , ) = formula.at(_index);
		uint256 tokenId = itemBase.createItem(class, grade, uint16(_prefer), uint16(_index), uint16(_rate), canDisenchant, _major, _tokens, _amounts, _owner);
		emit Smelt(_owner, tokenId, _index, _major, _tokens, _amounts);
	} 

	function disenchant(uint256 _tokenId, uint256 _depth) public stoppable returns (uint256) {
	   	return _disenchant(_tokenId, _depth);
	}

	function _disenchant(uint256 _tokenId, uint256 _depth) internal returns (uint256) {
		require(msg.sender == ownership.ownerOf(_tokenId), "Only owner can disenchant.");
		(bool canDisenchant, uint16 class, , , ) = itemBase.getSmeltInfo(_tokenId);
	...
	function _disenchantItem(address _owner, uint256 _tokenId) internal returns (uint256) {
		(bool canDisenchant, uint16 class, uint256 major, address[] memory tokens, uint256[] memory amounts) = itemBase.getSmeltInfo(_tokenId);
